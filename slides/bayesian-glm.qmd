...
- In some cases, using the wrong model is not only a problem of prediction but also a very serious inferential problem:
  - see @Liddell2018-wu for ordinal (e.g., likert)
  - see this [presentation](https://filippogambarota.github.io/slides/aip-sviluppo-2024/aip-sviluppo-2024.pdf) about type-1 error using wrong models 
...
## Extracting Posterior Distributions^[https://www.andrewheiss.com/blog/2022/09/26/guide-visualizing-types-posteriors/]
...
This is a really amazing package, have a look at the [`Features`](https://easystats.github.io/bayestestR/) section because contains functions and explanations about different ways of summarizing the posterior distribution.
...
We can calculate the cell-means for the Group, averaging over the level of the other factor i.e. the main effect. We can use the [`tidybayes`](https://mjskay.github.io/tidybayes/) package.
...
For an overview of the logistic regression we can refer to this [material](https://stat-teaching.github.io/GLMphd/slides/02-binomial-glm/02-binomial-glm.html#/title-slide).
...
#| code-fold: true
par(mfrow = c(1,2))
hist(post$b_Parental_stress)
hist(exp(post$b_Parental_stress))
```
...
```{r}
add_epred_draws(pp, fit_teddy) |> 
  ggplot(aes(x = Parental_stress, y = .epred)) +
  stat_halfeye()
```
...
- ROPE [~ to the Equivalence testing framework, @Lakens2017-mr; @Kruschke2018-ce]
- Bayesian p-values [@Makowski2019-tx]
- More on the [bayestestR](https://easystats.github.io/bayestestR/) package
...
- You should worry about multiple comparison [@Gelman2012-rm] when you do inference.
